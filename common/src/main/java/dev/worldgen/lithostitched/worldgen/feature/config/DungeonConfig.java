...
public record DungeonConfig(int minOpenings, int maxOpenings, IntProvider radius, int maxChests, WeightedList<EntityType<?>> spawnerMobs, BlockStateProvider floorProvider, BlockStateProvider wallProvider, TagKey<Block> dungeonInvalidBlocks, ResourceKey<LootTable> lootTable) implements FeatureConfiguration {
    ...
    private static final WeightedList<BlockState> DEFAULT_FLOOR = WeightedList.<BlockState>builder().add(Blocks.MOSSY_COBBLESTONE.defaultBlockState(), 3).add(Blocks.COBBLESTONE.defaultBlockState(), 1).build();
    ...
    public static final Codec<DungeonConfig> CODEC = RecordCodecBuilder.create(instance -> instance.group(
        ...
        WeightedList.wrappedCodec(BuiltInRegistries.ENTITY_TYPE.byNameCodec()).fieldOf("spawner_entity").orElse(DEFAULT_MOBS).forGetter(DungeonConfig::spawnerMobs),
